...
- [ ] `2026-09-06T00:10:00Z` — `claude` — `pnpm cf-typegen` with the installed wrangler (4.105) regenerates `worker-configuration.d.ts` against workerd 1.20260625 types, which break `tsc --noEmit` in unrelated places (`ExecutionContext.tracing`, `WorkflowEvent.workflowName`, `Env` losing the vars declared only in `src/env.d.ts`). The committed file predates that runtime. Bindings are typed through the `Cloudflare.Env` augmentation in `src/env.d.ts` instead; either pin the runtime types version the repo compiles against or stop tracking the generated file.
- [ ] `2026-09-06T00:10:00Z` — `claude` — Executor's FumaDB rejects a non-semver schema `version` with the bare message `the version 1 is invalid`, surfaced from the integration gateway's first D1 bring-up rather than at config time. `workers/integrations/src/executor.ts` pins `1.0.0`; the SDK type is a plain `string`, so nothing catches it earlier.
- [ ] `2026-09-06T01:05:00Z` — `claude` — On a cold start the integration gateway's first bootstrap (Google Discovery imports through the Executor SDK) logs two `Uncaught Error: Disallowed operation called within global scope` messages in `pnpm dev`. The catalog still registers and later calls succeed; `unhandledrejection`/`error` listeners in the worker catch nothing, so the source is a lazily imported chunk's module scope. Worth a bisect against `@executor-js/plugin-openapi` if it ever turns fatal on workerd.
